...
// }
  ...
  Future<void> fetchMagasinByRegionAndActeur(
      String idActeur, String idNiveau1Pays) async {
    // try {
    final response = await http.get(Uri.parse(
    ...
    if (response.statusCode == 200 ||
      ...
      magasinListe1 = data
          ...
          .map((item) => Magasin(
                ...
                niveau1Pays: Niveau1Pays(
                  idNiveau1Pays:
                      item['niveau1Pays']['idNiveau1Pays'] ?? 'manquant',
                  nomN1: item['niveau1Pays']['nomN1'] ?? 'manquant',
                  // Autres champs de l'acteur...
                ),
                dateAjout: item['dateAjout'] ?? 'manquante',
                localiteMagasin: item['localiteMagasin'] ?? 'manquante',
                statutMagasin:
                    item['statutMagasin'] as bool, // ou une valeur par défaut
              ))
          .toList();
      ...
      debugPrint(
          'Error Loading data! , Sever responded: ${response.statusCode}:${response.reasonPhrase.toString()}');
    }
  }
}
