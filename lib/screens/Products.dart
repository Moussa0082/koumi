...
class _ProductsScreenState extends State<ProductsScreen> {
  ...
  int page = 1;
   ...
   int size = 6;
  bool hasMore = true;
//  GridObserverController observerController = GridObserverController(controller: scrollableController);
  ...
  Future<List<Stock>> getAllStock() async {
    List<Stock> newStockList = [];
     if (selectedCat != null) {
      stockList = await StockService()
          .fetchProduitByCategorie(selectedCat!.idCategorieProduit!);
    }
    else if(widget.id != null) {
    stockList = await StockService().fetchStockByMagasin(widget.id!);
    
    }
     else if (selectedCat != null && widget.id != null) {
      stockList = await StockService().fetchProduitByCategorieAndMagasin(
          selectedCat!.idCategorieProduit!, widget.id!);
    }
  //    setState(() {
  //   stockListe.addAll(newStockList);
  // });
 ...
 if (scrollableController.position.pixels >=
      scrollableController.position.maxScrollExtent - 200 && hasMore && !isLoading) {
    // Nous sommes arrivés au dernier élément
    debugPrint("hiiiiii");
     setState(() {
...
//   try {
//     final response = await http.get(Uri.parse('$baseUrl/getAllStocksWithPagination?page=$page&size=$size'));
      
//     if (response.statusCode == 200 || response.statusCode == 201 || response.statusCode == 202) {
//       final jsonData = json.decode(response.body);
//       final List<dynamic> body = jsonData['content'];

//       // Convertir chaque élément du corps en Stock et les ajouter à la liste
//       setState(() {
//         stockListe.addAll(body.map((e) => Stock.fromMap(e)).toList());
//       });

//       debugPrint("response body all stock with pagination $size par defilement soit ${stockListe.length}");
//       return stockListe;
//     } else {
  ...
  Future<List<Stock>> fetchStock({bool refresh = false}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint("response body all stock with pagination $size par défilement soit ${stockListe.length}");
      } else {
        print('Échec de la requête avec le code d\'état: ${response.statusCode} |  ${response.body}');
      }
    } catch (e) {
  ...
  (typeActeurData
      ...
      body: RefreshIndicator(
        onRefresh:() async{
                  ...
                  // Rafraîchir les données ici
                  page = page + 1;
                  stockListeFuture = fetchStock(refresh:false);
                });
                              },
        child: Container(
