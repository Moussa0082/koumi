...
import 'package:koumi_app/days/Day.dart';
...
class _WeatherScreenState extends State<WeatherScreen> {
  ...
  if (!serviceEnabled) {
    setState(() {
      isError = true;
      error = "Les services de localisation sont désactivés. Veuillez les activer et redémarrer l'application.";
    });
    return;
  }
  ...
  if (permission == LocationPermission.denied) {
    ...
    if (permission == LocationPermission.denied) {
      setState(() {
        isError = true;
        error = "Les autorisations de localisation sont refusées. Veuillez les autoriser pour utiliser cette fonctionnalité.";
      });
      return;
    }
  }
  ...
  if (permission == LocationPermission.deniedForever) {
    setState(() {
      isError = true;
      error = "Les autorisations de localisation sont refusées de manière permanente. Veuillez les autoriser dans les paramètres de l'application.";
    });
    return;
  }
  ...
  getCityName() async {
    List<Placemark> placemarks = await placemarkFromCoordinates(12.577901, -8.062953);
  ...
  getWeatherData() async {
    ...
    try {
      final response = await http.get(Uri.parse(OpenWeatherAPI().apiUrl(12.577901, -8.062953)));
